...
func Test_SaveAlbumsGorm(t *testing.T) {
	t.Run("returns same ID for same spotify ID", func(t *testing.T) {
		brain := getInmemoryBrain(t)
		...
		want1 := []*MetaAlbum{bMetaAlbumHT, bMetaAlbumN}
		got1, err := brain.SaveAlbumsGorm([]*spotify.FullAlbum{sFullAlbumHT, sFullAlbumHT20, sFullAlbumN})
		if err != nil {
			t.Fatalf("got Error: %v", err)
		}
		if diff := diffMetaAlbums(want1, got1); diff != "" {
			t.Fatalf("SaveAlbumsGorm() mismatch (-want +got):\n%s", diff)
		}

		got2, err := brain.SaveAlbumsGorm([]*spotify.FullAlbum{sFullAlbumHT, sFullAlbumHT20, sFullAlbumN})
		if err != nil {
			t.Fatalf("got Error: %v", err)
		}
		if diff := diffMetaAlbums(want1, got2); diff != "" {
	...
	t.Run("returns different ID for different spotify ID", func(t *testing.T) {
		...
		want1 := []*MetaAlbum{bMetaAlbumHT}
		got1, err := brain.SaveAlbumsGorm([]*spotify.FullAlbum{sFullAlbumHT})
		if err != nil {
			t.Fatalf("got Error: %v", err)
		}
		if diff := diffMetaAlbums(want1, got1); diff != "" {
		...
		want2 := []*MetaAlbum{bMetaAlbumHT}
		got2, err := brain.SaveAlbumsGorm([]*spotify.FullAlbum{sFullAlbumHT20})
		if err != nil {
			t.Fatalf("got Error: %v", err)
		}
		if diff := diffMetaAlbums(want2, got2); diff != "" {
		...
		want3 := []*MetaAlbum{bMetaAlbumN}
		got3, err := brain.SaveAlbumsGorm([]*spotify.FullAlbum{sFullAlbumN})
		if err != nil {
			t.Fatalf("got Error: %v", err)
		}
		if diff := diffMetaAlbums(want3, got3); diff != "" {
